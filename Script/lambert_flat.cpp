...
int main()
{
    ...
    */
    ...
    glm::vec4 a = o + 1.7f*x + 0.6f*y + 8.3f*z;
    glm::vec4 b = o + 6.8f*x + 0.6f*y + 3.2f*z;
    glm::vec4 c = o + 2.1f*x + 4.0f*y + 1.0f*z;

    glm::vec4 p = 0.3f*a + 0.4f*b + 0.3f*c;

    glm::vec4 na = normalize(0.8f*x + 0.3f*y + 0.8f * z);
    glm::vec4 nb = normalize(0.5f * x + 0.5f * y + 0.8f * z);
    glm::vec4 nc = normalize(0.5f * x + 0.3f * y + 1.0f * z);

    glm::vec4 nt = normalize((b-a) * (c-a));
    ...
    glm::vec3 Kd = glm::vec3(0.9f, 0.6f, 0.7f); // Refletância difusa
    ...
    */
    ...
    float phong = pow(std::max(0.0f,dot(r,v)),q);
    ...
    printf("RGB - Flat shading --> [%f, %f, %f]",color.x, color.y, color.z);
    return 0;
} 
