...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 308054 - Giordano Souza de Paula", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    {
        if(wPressed)
           camera_position_c = Matrix_Translate(camera_view_vector.x*speed, camera_view_vector.y*speed, camera_view_vector.z*speed)*camera_position_c;
        if(sPressed)
            camera_position_c = Matrix_Translate(camera_view_vector.x*-speed, camera_view_vector.y*-speed, camera_view_vector.z*-speed)*camera_position_c;
        if(aPressed)
        {
            glm::vec4 orto = crossproduct(camera_up_vector, camera_view_vector);
            camera_position_c.x -= orto.x*-speed;
            camera_position_c.y -= orto.y*-speed;
            camera_position_c.z -= orto.z*-speed;
        }
        if(dPressed)
        {
            glm::vec4 orto = crossproduct(camera_up_vector, camera_view_vector);
        ...
        camera_view_vector = glm::vec4(-cos(g_CameraPhi)*sin(g_CameraTheta), -sin(g_CameraPhi), -cos(g_CameraPhi)*cos(g_CameraTheta), 0.0);
