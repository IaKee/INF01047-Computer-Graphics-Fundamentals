...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 308054 - Giordano Souza", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //CABEÇÇ
        //PushMatrix(model);
        //    model = model * Matrix_Translate(0.0f, 0.35f, 0.0f);
        //    model = model * Matrix_Scale(0.3f, 0.3f, 0.3f);
        //    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        //    DrawCube(render_as_black_uniform); // #### CABEÇÇ
        //PopMatrix(model);

        
        PushMatrix(model); //Perna Esq
            model = model * Matrix_Translate(-0.2f, -1.05f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.3f, 0.7f, 0.3f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform); // #### COXA
            PopMatrix(model);
            model = model * Matrix_Translate(0.0f, -0.75f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.25f, 0.7f, 0.25f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform); // #### CANELA
            PopMatrix(model);
            model = model * Matrix_Translate(0.0f, -0.75f, 0.1f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.25f, 0.1f, 0.5f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform); // #### PÉ
            PopMatrix(model);
        PopMatrix(model);

        PushMatrix(model); //Perna Dir
            model = model * Matrix_Translate(0.2f, -1.05f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.3f, 0.7f, 0.3f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        PushMatrix(model); // Braço dir - Guardamos matriz model atual na pilha
        ...
        PushMatrix(model); //Cabeça
            model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
                        * Matrix_Rotate_Z(g_AngleZ)  // TERCEIRO rotação Z de Euler
                        * Matrix_Rotate_Y(3.14f - g_AngleY)  // SEGUNDO rotação Y de Euler
                        * Matrix_Rotate_X(3.14f + g_AngleX); // PRIMEIRO rotação X de Euler
