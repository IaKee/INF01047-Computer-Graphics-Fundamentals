...
GLuint BuildTriangles()
{
    ...
    for(int i=0; i<32; i++)
    {
        if(i%2==0)
        {
            ...
            //Ouser
            NDC_coefficients[i*4]=0.7f*sin((PI/16)*i);
            NDC_coefficients[i*4+1]=0.7f*cos((PI/16)*i);
        ...
        {
            //Se for impar
            NDC_coefficients[i*4]=0.5f*sin((PI/16)*(i-1));
            NDC_coefficients[i*4+1]=0.5f*cos((PI/16)*(i-1));;
            NDC_coefficients[i*4+2]=0.0f;
            NDC_coefficients[i*4+3]=1.0f;
        }
        printf("Indice %d - %f %f %f \n", i, NDC_coefficients[i*4], NDC_coefficients[i*4+1], NDC_coefficients[i*4+2]);
    }
    ...
    for(int i=0; i<32; i++)
    {
        ...
        {
            color_coefficients[i*4]=0.0f; //R
            color_coefficients[i*4+1]=0.0f; //G
            color_coefficients[i*4+2]=1.0f; //B
            color_coefficients[i*4+3]=1.0f; //A
        }
    }
    ...
    GLubyte indices[] = {0, 1, 2,
                               ...
                               31,
                               0,
                               1 }; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
